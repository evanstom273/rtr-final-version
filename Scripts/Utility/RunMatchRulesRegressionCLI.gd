...
func _run() -> void:
	...
	_check(normal.action_clock_seconds == 15, "Normal action clock step should default to fifteen seconds.")
	...
	_check(invalid_clock.action_clock_seconds == 15, "Unsupported clock steps should fall back to fifteen seconds.")
