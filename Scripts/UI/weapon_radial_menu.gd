...
func _rebuild() -> void:
	...
	for entry in _entries:
		...
		style.bg_color = AppThemePalette.with_alpha(AppThemePalette.SECONDARY_PANEL, 0.98)
		style.border_color = AppThemePalette.with_alpha(AppThemePalette.BORDER, 0.95)
		...
		title.add_theme_color_override("font_color", AppThemePalette.PRIMARY_TEXT)
		title.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
		labels.add_child(title)
		var stats := Label.new()
		stats.text = "IMPACT %s  •  USES %s" % [entry.get("impact", "—"), entry.get("durability", "—")]
		stats.add_theme_color_override("font_color", AppThemePalette.SECONDARY_TEXT)
		stats.add_theme_font_size_override("font_size", 12)
		labels.add_child(stats)
		var warning := Label.new()
		warning.text = str(entry.get("warning", ""))
		warning.add_theme_color_override("font_color", AppThemePalette.WARNING)
